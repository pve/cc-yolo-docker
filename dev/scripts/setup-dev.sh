...
cd "${WORKSPACE}"
envsubst < /opt/cc/CLAUDE.md.template > CLAUDE.md
if git diff --quiet CLAUDE.md 2>/dev/null && git ls-files --error-unmatch CLAUDE.md 2>/dev/null; then
    echo "    CLAUDE.md unchanged — skipping commit."
else
    git add CLAUDE.md
    git commit -m "chore: update CLAUDE.md for cc-dev environment
